...
@app.route('/dope_shit/', methods=['POST', 'GET'])
def dope_shit():
    '''404 URL, nothing special'''
    return render_template('vocab_table/404_page.html')
...
def get_message():
    ...
    try:
        ...
        return redirect(url_for('dope_shit'))
...
#        fdf
        ...
        print(ex)
        config_file.ex_catcher(current_user.id, 'render_table', str(ex))
        return redirect(url_for('dope_shit'))
...
def intermediate(id):
    ...
    try:
        ...
        config_file.ex_catcher(current_user.id, 'intermediate', ex)
        return redirect(url_for('dope_shit'))
...
def modificateWordInTable(word, id, definition):
    ...
    try:
        ...
        config_file.ex_catcher(current_user.id, 'modificateWordInTable', ex)
        return redirect(url_for('dope_shit'))
